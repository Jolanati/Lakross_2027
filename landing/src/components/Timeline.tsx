  ...
  return (
    <div className="py-20 px-4 bg-charcoal">
      <div className="max-w-5xl mx-auto">
        ...
        <h2 className="text-3xl sm:text-4xl font-display font-black text-cream text-center mb-16">
          {t.timeline.title}
        </h2>
            ...
            {t.timeline.months.map((item, i) => {
              ...
              return (
                <div key={i} className={`relative flex items-start gap-8 ${isLeft ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                  {/* Dot */}
                  <div className="absolute left-4 md:left-1/2 w-4 h-4 bg-carmine rounded-full -translate-x-1/2 border-4 border-charcoal z-10" />

                  {/* Spacer for desktop */}
                  <div className="hidden md:block w-1/2" />

                  {/* Card */}
                  <div className="ml-12 md:ml-0 md:w-1/2 bg-[#1f1f1f] rounded-xl p-6 border border-white/10">
                    <div className="flex items-center gap-3 mb-3">
                      <span className="text-xs font-bold text-white bg-carmine px-3 py-1 rounded-full">
                        {item.month}
                      </span>
                      <span className="text-sm font-semibold text-cream/50">{item.label}</span>
                    </div>
